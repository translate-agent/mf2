...
func (e *executer) resolveExpression(expr ast.Expression) (*ResolvedValue, error) {
	...
	var (
		funcName      string
		options       map[string]*ResolvedValue
		resolutionErr error
	)
...
func (e *executer) resolveOptions(options []ast.Option) (map[string]*ResolvedValue, error) {
	m := make(map[string]*ResolvedValue, len(options))
